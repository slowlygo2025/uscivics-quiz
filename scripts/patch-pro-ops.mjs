/**
 * Pro-ops dict + privacy patch:
 * - remove premiumWaitlist*
 * - add account reset/delete strings
 * - align privacy "provide" + "choices" for all locales
 */
import { readFileSync, writeFileSync, readdirSync } from "node:fs";
import { join } from "node:path";

const root = process.cwd();
const dictDir = join(root, "src/lib/dict");

const WAITLIST_RE =
  /\n\s*premiumWaitlistTitle:[\s\S]*?premiumWaitlistDone:[^\n]*,\n/;

const ACCOUNT_KEYS_EN = `  accountResetPassword: "Reset password",
...
const ACCOUNT_BY_LOCALE = {
  en: ACCOUNT_KEYS_EN,
  es: `  accountResetPassword: "Restablecer contraseña",
...
`,
  zh: `  accountResetPassword: "重置密码",
  accountResetSent: "已发送重置邮件 — 请查收收件箱。",
  accountDelete: "删除账户",
  accountDeleteConfirm: "删除账户和云端进度？本设备上的本地进度会保留，直到您清除网站数据。",
  accountDeleted: "账户已删除。",
`,
  vi: `  accountResetPassword: "Đặt lại mật khẩu",
...
`,
  tl: `  accountResetPassword: "I-reset ang password",
  accountResetSent: "Naipadala ang reset email — tingnan ang inbox.",
  accountDelete: "Burahin ang account",
  accountDeleteConfirm: "Burahin ang account at cloud progress? Mananatili ang local progress sa device na ito hanggang i-clear mo ang site data.",
  accountDeleted: "Nabura ang account.",
`,
  ar: `  accountResetPassword: "إعادة تعيين كلمة المرور",
  accountResetSent: "تم إرسال بريد إعادة التعيين — تحقق من صندوق الوارد.",
  accountDelete: "حذف الحساب",
  accountDeleteConfirm: "حذف حسابك وتقدّم السحابة؟ يبقى التقدّم المحلي على هذا الجهاز حتى تمسح بيانات الموقع.",
  accountDeleted: "تم حذف الحساب.",
`,
  ko: `  accountResetPassword: "비밀번호 재설정",
  accountResetSent: "재설정 이메일을 보냈습니다 — 받은편지함을 확인하세요.",
  accountDelete: "계정 삭제",
  accountDeleteConfirm: "계정과 클라우드 진도를 삭제할까요? 이 기기의 로컬 진도는 사이트 데이터를 지울 때까지 남습니다.",
  accountDeleted: "계정이 삭제되었습니다.",
`,
  hi: `  accountResetPassword: "पासवर्ड रीसेट",
  accountResetSent: "रीसेट ईमेल भेजा गया — इनबॉक्स देखें।",
  accountDelete: "खाता हटाएँ",
  accountDeleteConfirm: "अपना खाता और क्लाउड प्रगति हटाएँ? इस डिवाइस की स्थानीय प्रगति साइट डेटा साफ़ करने तक रहेगी।",
  accountDeleted: "खाता हटा दिया गया।",
`,
  ru: `  accountResetPassword: "Сбросить пароль",
  accountResetSent: "Письмо для сброса отправлено — проверьте почту.",
  accountDelete: "Удалить аккаунт",
  accountDeleteConfirm: "Удалить аккаунт и облачный прогресс? Локальный прогресс на этом устройстве останется, пока вы не очистите данные сайта.",
  accountDeleted: "Аккаунт удалён.",
`,
  ht: `  accountResetPassword: "Reyajiste modpas",
  accountResetSent: "Imel reyajisman voye — tcheke bwat resepsyon ou.",
  accountDelete: "Efase kont",
  accountDeleteConfirm: "Efase kont ou ak pwogrè nwaj la? Pwogrè lokal sou aparèy sa a rete jiskaske w efase done sit la.",
  accountDeleted: "Kont efase.",
`,
  fr: `  accountResetPassword: "Réinitialiser le mot de passe",
  accountResetSent: "E-mail de réinitialisation envoyé — vérifiez votre boîte.",
  accountDelete: "Supprimer le compte",
  accountDeleteConfirm: "Supprimer votre compte et la progression cloud ? La progression locale sur cet appareil reste jusqu’à ce que vous effaciez les données du site.",
  accountDeleted: "Compte supprimé.",
`,
};

for (const file of readdirSync(dictDir).filter((f) => f.endsWith(".ts"))) {
  const locale = file.replace(/\.ts$/, "");
  const path = join(dictDir, file);
  let src = readFileSync(path, "utf8");
  src = src.replace(WAITLIST_RE, "\n");
  if (!src.includes("accountResetPassword:")) {
    const insert = ACCOUNT_BY_LOCALE[locale] || ACCOUNT_BY_LOCALE.en;
    if (!src.includes("accountUnavailable:")) {
      console.error("missing accountUnavailable in", locale);
      process.exit(1);
    }
    src = src.replace(
      /(\s*accountUnavailable:\s*[^\n]+,\n)/,
      `$1${insert}`
    );
  }
...
const privacyChoices = {
  en: `You may clear local site data in your browser, block cookies, or email ${"${CONTACT_EMAIL}"} for privacy-related questions. If you created an optional account, you can reset your password or delete the account (and associated cloud progress) from the practice sync panel, or email us to request deletion. Local progress on a device remains until you clear site data.`,
  es: `Podés borrar datos del sitio en el navegador, bloquear cookies o escribir a ${"${CONTACT_EMAIL}"} por temas de privacidad. Si creaste una cuenta opcional, podés restablecer la contraseña o eliminar la cuenta (y el progreso en la nube) desde el panel de sync en práctica, o pedirnos el borrado por email. El progreso local en un dispositivo queda hasta que borres los datos del sitio.`,
  zh: `您可以清除浏览器中的本地网站数据、屏蔽 Cookie，或就隐私相关问题发邮件至 ${"${CONTACT_EMAIL}"}。如果您创建了可选账户，可在练习页的同步面板中重置密码或删除账户（及关联的云端进度），也可发邮件请求删除。设备上的本地进度会保留，直到您清除网站数据。`,
  vi: `Bạn có thể xóa dữ liệu trang cục bộ trong trình duyệt, chặn cookie, hoặc gửi email tới ${"${CONTACT_EMAIL}"} về quyền riêng tư. Nếu đã tạo tài khoản tùy chọn, bạn có thể đặt lại mật khẩu hoặc xóa tài khoản (và tiến trình đám mây) từ bảng đồng bộ trên trang luyện tập, hoặc email để yêu cầu xóa. Tiến trình cục bộ trên thiết bị vẫn còn cho đến khi bạn xóa dữ liệu trang.`,
  tl: `Maaari mong i-clear ang lokal na site data sa browser, harangan ang cookies, o mag-email sa ${"${CONTACT_EMAIL}"} para sa privacy. Kung may optional account ka, maaari mong i-reset ang password o burahin ang account (at cloud progress) mula sa sync panel sa practice, o mag-email para humiling ng pagbura. Ang local progress sa device ay nananatili hanggang i-clear mo ang site data.`,
  ...
  ko: `브라우저에서 로컬 사이트 데이터를 지우거나, 쿠키를 차단하거나, 개인정보 문의는 ${"${CONTACT_EMAIL}"}로 이메일을 보낼 수 있습니다. 선택 계정을 만들었다면 연습 페이지의 동기화 패널에서 비밀번호를 재설정하거나 계정(및 클라우드 진도)을 삭제할 수 있으며, 이메일로 삭제를 요청할 수도 있습니다. 기기의 로컬 진도는 사이트 데이터를 지울 때까지 남습니다.`,
...
// Fix the template - I used wrong escaping. CONTACT_EMAIL is a JS template in site-pages
function choicesBody(locale) {
  const map = {
    en: `You may clear local site data in your browser, block cookies, or email \${CONTACT_EMAIL} for privacy-related questions. If you created an optional account, you can reset your password or delete the account (and associated cloud progress) from the practice sync panel, or email us to request deletion. Local progress on a device remains until you clear site data.`,
    es: `Podés borrar datos del sitio en el navegador, bloquear cookies o escribir a \${CONTACT_EMAIL} por temas de privacidad. Si creaste una cuenta opcional, podés restablecer la contraseña o eliminar la cuenta (y el progreso en la nube) desde el panel de sync en práctica, o pedirnos el borrado por email. El progreso local en un dispositivo queda hasta que borres los datos del sitio.`,
    zh: `您可以清除浏览器中的本地网站数据、屏蔽 Cookie，或就隐私相关问题发邮件至 \${CONTACT_EMAIL}。如果您创建了可选账户，可在练习页的同步面板中重置密码或删除账户（及关联的云端进度），也可发邮件请求删除。设备上的本地进度会保留，直到您清除网站数据。`,
...
const sitePath = join(root, "src/lib/site-pages.ts");
let site = readFileSync(sitePath, "utf8");

// Update last-updated dates in privacy overview (EN + ES samples + generic)
...
const provideOld = {
  ...
  es: "Podés usar el sitio sin crear una cuenta. Si nos escribís por email, recibimos la dirección y el contenido del mensaje. Datos opcionales como un ZIP para buscar oficiales se usan para devolver resultados de estudio y no para armar un perfil de marketing.",
  zh: "您无需创建账户即可使用本网站。如果您给我们发邮件，我们会收到您发送的邮箱地址和邮件内容。诸如用于查询民选官员的邮政编码（ZIP）等可选输入信息，仅用于返回学习结果，不会用于构建营销画像。",
  vi: "Bạn có thể sử dụng trang web mà không cần tạo tài khoản. Nếu bạn gửi email cho chúng tôi, chúng tôi sẽ nhận được địa chỉ và nội dung tin nhắn bạn gửi. Các thông tin tùy chọn như mã ZIP dùng để tra cứu quan chức được xử lý để trả về kết quả học tập và không được dùng để xây dựng hồ sơ tiếp thị.",
  tl: "Magagamit mo ang site nang hindi gumagawa ng account. Kung mag-e-email ka sa amin, matatanggap namin ang address at nilalaman ng mensaheng ipinadala mo. Ang mga opsyonal na input tulad ng ZIP code na ginagamit para sa paghahanap ng mga opisyal ay pinoproseso para magbalik ng resulta ng pag-aaral at hindi ginagamit para bumuo ng marketing profile.",
  ar: "يمكنك استخدام الموقع دون إنشاء حساب. إذا راسلتنا عبر البريد الإلكتروني، فسنستلم عنوانك ومحتوى الرسالة التي ترسلها. تتم معالجة المدخلات الاختيارية مثل الرمز البريدي المستخدم للبحث عن المسؤولين المنتخبين لإرجاع نتائج الدراسة فقط، ولا تُستخدم لإنشاء ملف تسويقي.",
  ko: "계정을 만들지 않고도 사이트를 이용할 수 있습니다. 이메일을 보내주시면 발신 주소와 메시지 내용을 수신하게 됩니다. 선출직 공무원 조회에 사용되는 우편번호와 같은 선택적 입력 정보는 학습 결과를 제공하기 위해 처리되며 마케팅 프로필 작성에는 사용되지 않습니다.",
  hi: "आप बिना खाता बनाए साइट का उपयोग कर सकते हैं। यदि आप हमें ईमेल करते हैं, तो हमें आपका भेजा गया पता और संदेश की सामग्री प्राप्त होती है। निर्वाचित अधिकारियों की खोज के लिए उपयोग किए जाने वाले ZIP कोड जैसे वैकल्पिक इनपुट को अध्ययन परिणाम लौटाने के लिए संसाधित किया जाता है और इनका उपयोग मार्केटिंग प्रोफ़ाइल बनाने के लिए नहीं किया जाता।",
  ...
  ht: "Ou ka itilize sit la san w pa kreye yon kont. Si w voye yon imel ba nou, nou resevwa adrès ak kontni mesaj ou voye a. Enfòmasyon opsyonèl tankou yon kòd postal (ZIP) ki itilize pou chèche ofisyèl eli yo trete pou retounen rezilta etid; nou pa itilize yo pou konstwi yon pwofil maketing.",
...
const choicesOld = {
  en: "You may clear local site data in your browser, block cookies, or email ${CONTACT_EMAIL} for privacy-related questions. Because we do not require accounts, we may have limited personal data beyond what you email us or what providers store in logs/analytics.",
  es: "Podés borrar datos del sitio en el navegador, bloquear cookies o escribir a ${CONTACT_EMAIL} por temas de privacidad. Como no exigimos cuentas, podemos tener pocos datos personales más allá de lo que nos envíes por email o lo que guarden logs/analítica de proveedores.",
  zh: "您可以清除浏览器中的本地网站数据、屏蔽 Cookie，或就隐私相关问题发邮件至 ${CONTACT_EMAIL}。由于我们不要求注册账户，除了您通过邮件发送给我们的信息，或服务提供商在日志/分析中存储的信息外，我们掌握的个人数据可能非常有限。",
  vi: "Bạn có thể xóa dữ liệu trang web cục bộ trong trình duyệt, chặn cookie, hoặc gửi email đến ${CONTACT_EMAIL} cho các câu hỏi liên quan đến quyền riêng tư. Vì chúng tôi không yêu cầu tài khoản, chúng tôi có thể có rất ít dữ liệu cá nhân ngoài những gì bạn gửi qua email hoặc những gì nhà cung cấp lưu trong nhật ký/phân tích.",
  tl: "Maaari mong i-clear ang lokal na data ng site sa iyong browser, harangan ang cookies, o mag-email sa ${CONTACT_EMAIL} para sa mga tanong tungkol sa privacy. Dahil hindi kami humihingi ng account, maaaring limitado ang aming personal na data maliban sa ipinadala mo sa amin sa email o iniimbak ng mga provider sa logs/analytics.",
  ar: "يمكنك مسح بيانات الموقع المحلية في متصفحك، أو حظر ملفات تعريف الارتباط، أو مراسلتنا عبر ${CONTACT_EMAIL} للأسئلة المتعلقة بالخصوصية. نظراً لأننا لا نطلب حسابات، فقد تكون لدينا بيانات شخصية محدودة بخلاف ما ترسله إلينا عبر البريد الإلكتروني أو ما يخزنه مزودو الخدمة في السجلات/التحليلات.",
  ko: "귀하는 브라우저에서 로컬 사이트 데이터를 지우거나, 쿠키를 차단하거나, 개인정보 관련 문의를 위해 ${CONTACT_EMAIL}로 이메일을 보낼 수 있습니다. 저희는 계정을 요구하지 않으므로, 귀하가 이메일로 보낸 정보나 제공업체가 로그/분석에 저장하는 정보 외에는 보유하는 개인정보가 제한적일 수 있습니다.",
  hi: "आप अपने ब्राउज़र में स्थानीय साइट डेटा साफ़ कर सकते हैं, कुकीज़ को ब्लॉक कर सकते हैं, या गोपनीयता संबंधी प्रश्नों के लिए ${CONTACT_EMAIL} पर ईमेल कर सकते हैं। चूंकि हमें खातों की आवश्यकता नहीं है, इसलिए आपके द्वारा हमें ईमेल की गई जानकारी या प्रदाताओं द्वारा लॉग/एनालिटिक्स में संग्रहीत जानकारी से परे हमारे पास सीमित व्यक्तिगत डेटा हो सकता है।",
  ...
  ht: "Ou ka efase done lokal sit la nan navigatè ou, bloke cookies, oswa voye yon imel bay ${CONTACT_EMAIL} pou kesyon sou vi prive. Piske nou pa egzije kont, nou ka gen done pèsonèl limite apa de sa w voye ba nou pa imel oswa sa founisè yo estoke nan jounal/analiz.",
...
for (const loc of Object.keys(provideOld)) {
  if (!site.includes(provideOld[loc])) {
    console.warn("provide old not found", loc);
  } else {
    site = site.replace(provideOld[loc], privacyProvide[loc]);
  }
  if (!site.includes(choicesOld[loc])) {
    console.warn("choices old not found", loc);
  } else {
    site = site.replace(choicesOld[loc], choicesBody(loc));
  }
}

// Sharing EN: mention Firebase + Resend
site = site.replace(
